...
const login = (req, res) => {
  ...
  Chauffeur.findOne({ email: email })
    ...
    .catch(err => {
      console.log(err);
      res.status(500).send({ message: "Error retrieving user with username " + email });
    });
};
